...
void graph::DFS_shortPath(){
	vector<edge*>* edgePath = new vector<edge*>();
	vector<int>* vertexPath = new vector<int>();
...
void graph::dfs_shortPath(vertex * currentVertex, int outdegree, vector<int>* vertexPath, vector<edge*>* edgePath){
	...
	if (nextVertex->VId() != end_id) {//不是终点
		if (nextVertex->getEdgeQuantity() > 0) {//出度大于0
			nextVertex->setStatus(INSTACK);//标记点入栈
			vertexPath->push_back(nextVertex->VId());//入栈操作
			edge* nextEdge = currentVertex->getEdge(outdegree);//get经过的边
			edgePath->push_back(nextEdge);
			//cout << nextEdge->getEId() << ' ';
			nextEdge->setStatus(RECORDED);//标记边入栈
			for (int i = 0; i < nextVertex->getEdgeQuantity(); i++) {//针对栈顶的节点
				vertex* endVertex = Map.find(nextVertex->getEdge(i)->getEndId())->second;//get栈顶节点的下一目的地
				if (endVertex->status() == CANUSE) {//若下一目的地不在栈里
					dfs_shortPath(nextVertex, i, vertexPath, edgePath);//递归深入
					endVertex->setStatus(CANUSE);
				}
			}
			nextEdge->setStatus(AVAILABLE);
			nextVertex->setStatus(CANUSE);//调整状态
			vertexPath->pop_back();
		...
		edgePath->push_back(currentVertex->getEdge(outdegree));
...
void graph::judgePath(vector<int>* vertexPath, vector<edge*>* edgePath){//当路径尾是终点时会调用此函数
	...
	if (p_size < vertexPath->size()) {//路径节点数要大于path中节点数量
		...
		if (final_path->empty() || p_weight < MinWeight) {//如果图类中的路径存在
			...
			for (vector<edge*>::iterator it = edgePath->begin(), ed = edgePath->end(); it != ed; it++)
				final_path->push_back((*it)->getEId());//更新final_path
			MinWeight = p_weight;
		}
...
//构造函数graph的作用，接受两个.csv文件的路径，初始化点集和边集，初始化MustPassing点集，
	...
	while (!outFile.eof()) {//一行一行地读
		...
		std::getline(outFile, buffer);//读取文件中的一行到buffer中
